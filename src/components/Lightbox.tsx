...
const Lightbox = ({ closeLightbox, initialSlide, children }: LightboxProps) => {
  ...
  return (
    <div
      ...
      className={`w-full h-screen fixed top-0 left-0 z-10 transition duration-300 flex justify-center items-center ${
        entered ? 'bg-black/[0.85]' : 'bg-transparent'
      }`}
    >
      {sliderEntered && (
        <Slider
          ...
          className={`aspect-[3/4] max-w-[80%] max-h-[95%] ${sliderClass}`}
        >
          {children}
        </Slider>
      )}
